...
import { getWeekStart, calculateDuration, formatDateTimeForInput, parseDateTimeInput } from '../../utils';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // Update adminWeekStart if settings change and we are on the default view
  useEffect(() => {
  ...
  // Hardcoded Temp Agencies for Dropdown (In a real app, these might come from Settings)
  ...
  // Update split name state when editingEmployee changes
  useEffect(() => {
      if (editingEmployee) {
  ...
  useEffect(() => {
    if (isAddingMode && editingEmployee) {
        ...
        if (editingEmployee.isTemp && editingEmployee.tempAgency) {
            // Use Temp Agency Name as prefix (e.g. "StaffingPro-")
        ...
        if (prefix) {
            ...
            // 1. Check Location Abbreviations
            ...
            // 2. Check Temp Agency Prefixes (simplified 3 letter logic)
                    ...
                    {sortedWeekKeys.map(weekKey => (
                      <div key={weekKey} className="mb-8 bg-slate-800 rounded-lg p-1">
                        <div className="flex justify-between items-center bg-slate-900 p-4 rounded-t-lg">
                          ...
                          <button onClick={() => exportWeekToCsv(recordsByWeek[weekKey], weekKey)} className="flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white font-bold py-2 px-3 rounded-lg transition">
