...
// Ionic Core CSS required for Web
...
import { initializeCapacitorPlugins } from './plugins/capacitor-setup';

// Initialize Ionic
setupIonicReact({
  mode: 'md', // Material Design mode for consistent Android/Web look
});

// --- MAIN APP COMPONENT ---

const App: React.FC = () => {
  // Replace local mock data hook with RxDB hook
  const {
    employees,
    locations,
    timeRecords,
  ...
  // Set initial selected location once locations load
  useEffect(() => {
  ...
  // Refactored Handlers to use DB Actions
